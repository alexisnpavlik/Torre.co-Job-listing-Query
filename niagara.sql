/* AA : Sonic : Niagara : prod */ 
SELECT
    `source`.`ref_id` AS `Alfa ID`,
    `source`.`active` AS `active`,
    `source`.`compensation_status` AS `compensation_status`,
    `source`.`channels` AS `channels`,
    `source`.`current_state` AS `current_state`
FROM
    (
        SELECT
            `opportunity`.`id` AS `id`,
            `opportunity`.`ref_id` AS `ref_id`,
